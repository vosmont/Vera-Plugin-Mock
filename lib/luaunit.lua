...
Initial rework: Philippe Fremy <phil@freehackers.org>
Homepage: http://phil.freehackers.org/programs/luaunit/

Current: vosmont
https://github.com/vosmont/Vera-Plugin-Mocks

Version: 1.4
License: X11 License, see LICENSE.txt

Changes between 1.4 and 1.3 (vosmont):
- add alias methods as JUnit and QUnit
- add 'expect' method
...
function assertEquals(actual, expected, message)
	...
	nbAssertions = nbAssertions + 1
	if (type(message) == "string") then
		print("ASSERT#" .. tostring(nbAssertions) .. ": " .. message)
	else
		print("ASSERT#" .. tostring(nbAssertions))
	end
	--if ((type(actual) ~= type(expected)) or (actual ~= expected))  then
...
function assertNull (actual, message)
	assertEquals(actual, nil, message)
end
...
LuaUnit = {
	...
	function LuaUnit:runTestMethod(aName, aClassInstance, aMethod)
		...
		if ((nbExpectedAssertions > -1) and (nbAssertions ~= nbExpectedAssertions)) then
			LuaUnit.result:addFailure( "Number of expected assertion not reached" )
		end
